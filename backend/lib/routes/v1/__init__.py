...
@bp1.errorhandler(500)
def server_error(e):
    _logger.error(f'Internal Error: {e}')
    return jsonify(ErrorResponse(message='Internal Server Error :(', code=500).model_dump()), 500


from . import random, info, image, definition, latest, search, node, ratings, aicg_chronicles, health, chats, user, webring, white_ethnostate, card_ideas